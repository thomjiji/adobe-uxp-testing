...
const ppro: premierepro = require("premierepro");
// @ts-ignore - UXP uses require for module loading
...
const logSuccess = (msg: string): void => log(`- ${msg}`, "#00ff00");
const logError = (msg: string): void => log(`- ${msg}`, "#ff0000");
const logWarning = (msg: string): void => log(`- ${msg}`, "#ffaa00");
const logInfo = (msg: string): void => log(`- ${msg}`, "#aaaaaa");
 ...
 */
async function getActiveProjectSafe(): Promise<Project | null> {
  try {
 ...
 */
async function getActiveSequenceSafe(): Promise<Sequence | null> {
  try {
 ...
 */
async function formatTickTime(tickTime: TickTime): Promise<string> {
  try {
 ...
 */
async function relinkSourceBinMedia(
  oldBasePath: string,
  newBasePath: string
