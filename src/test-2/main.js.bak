    ...
    try {
        ...
        for (let i = 0; i < trackCount; i++) {
            ...
            const trackItems = track.getTrackItems(
                ppro.VideoClipTrackItem.TRACKITEMTYPE_CLIP,
                false
            );
            ...
            for (let j = 0; j < Math.min(trackItems.length, 5); j++) {
                ...
                log(
                    `  ${j + 1}. ${name} [${await formatTickTime(start)} - ${await formatTickTime(
                        end
                    )}]`
                );
            }
 ...
 *
 * Available globals:
 * - ppro: The Premiere Pro API object
        ...
        for (const item of items) {
            ...
            if (item.type === 2) {
                ...
                if (isSeq) {
                    ...
                    // It's a regular clip
                    clipCount++;
                    const changedSuccess = await clipItem.changeMediaFilePath(
                        "/Users/geekshootjack/git/adobe-uxp-testing/media/footage/250712_zbcjl_20117.MP4"
                    );
...
async function relinkSourceBinMedia(oldBasePath, newBasePath) {
    const project = await getActiveProjectSafe();
    if (!project) return { success: 0, failed: 0, errors: [] };
    const results = {
        success: 0,
        failed: 0,
        errors: [],
                    ...
                    if (!isSeq) {
                        ...
                        try {
                            ...
                            if (normalizedOldPath.startsWith(normalizedOldBase)) {
                                const relativePath = normalizedOldPath.substring(
                                    normalizedOldBase.length
                                );
                                ...
                                if (success) {
                                    ...
                                    results.failed++;
                                    results.errors.push(
                                        `${item.name}: changeMediaFilePath returned false`
                                    );
                                    log(`  Result: FAILED`);
                                }
                            } else {
                                log(
                                    `Skipping ${item.name}: path does not start with old base path`
                                );
...
async function runCustomTest() {
    ...
    await relinkSourceBinMedia(
        "D:\\25076-木马乐队×白敬亭「Feifei Run」LIVE版MV\\素材\\",
        "/Volumes/GSJ#554/25076-木马乐队×白敬亭「Feifei Run」LIVE版MV/素材/"
    );
}

async function metadataGrabber() {
    clearLog();
    const project = await getActiveProjectSafe();
    const rootItem = await project.getRootItem();
    const items = await rootItem.getItems();
    for (const item of items) {
        if (item.type === 1) {
            const metadata = await ppro.Metadata.getProjectColumnsMetadata(item);
            log(`we got metadata finally: ${metadata}`);
        } else if (item.type === 2) {
            log(`item type 2`)
        } else {
            log(`something else`)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (runCustomBtn) {
        runCustomBtn.addEventListener("click", metadataGrabber);
    }
    if (clearBtn) {
